...
const BrandLockup = ({ light = false }) => (
  <div className={`${styles.loginBrand} ${light ? styles.loginBrandLight : ''}`}>
    <Image src={light ? BRAND_LOGOS.onDark.icon : BRAND_LOGOS.onLight.icon} alt="" width={54} height={72} priority={light} />
    <div>
      <strong>Scrum Tracker</strong>
      <span>by Systems Edge Solutions</span>
    </div>
  </div>
);
...
export default function LoginPage() {
  ...
  return <div className={`${styles.authPage} ${styles.loginPage}`}>
    <aside className={`${styles.authHero} ${styles.loginHero}`}>
      ...
      <div className={`${styles.heroContent} ${styles.loginHeroContent}`}>
        <BrandLockup light />
        <h1 className={styles.loginHeroTitle}>Keep every sprint<br />moving forward.</h1>
    ...
    <main className={`${styles.authForm} ${styles.loginFormPanel}`}>
      <div className={`${styles.formContainer} ${styles.loginCard}`}>
        ...
        <form className={styles.loginFormFields} onSubmit={handleSubmit}>
          ...
          <button type="submit" className={styles.loginSubmit} disabled={loading || googleLoading}>{loading ? 'Signing in…' : 'Sign in'}</button>
        </form>
        <p className={`${styles.formFooter} ${styles.loginFooter}`}>New to Scrum Tracker? <Link href="/register" className={styles.formLink}>Create an account</Link></p>
      </div>
    </main>
  </div>;
}
